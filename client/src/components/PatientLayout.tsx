...
export const PatientLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-hidden dashboard-theme">
      ...
      {/* Patient Collapsible Sidebar (Vercel pure black styling) */}
      <div
        className={clsx(
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-5 scrollbar-thin">
          <div>
            {!isCollapsed ? (
              <p className="px-3 mb-2 text-[9px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">My Health</p>
            ) : (
            ...
            <div className="space-y-0.5">
              {patientMenuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {active && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 bg-cyan-500 dark:bg-white rounded-r-full" />
                    )}
                    <item.icon className={clsx(
                      "w-[18px] h-[18px] flex-shrink-0 transition-transform duration-200",
                      active ? "text-cyan-600 dark:text-white" : "group-hover:scale-105 text-slate-400 dark:text-zinc-500 group-hover:text-slate-700 dark:group-hover:text-zinc-200"
                    )} />
        ...
        {/* Sidebar Footer (Vercel Avatar box) */}
          ...
          {!isCollapsed ? (
            <div className="mt-2 p-2.5 rounded-xl border border-slate-200/50 dark:border-zinc-800/80 bg-white dark:bg-[#000000] flex items-center justify-between gap-2 shadow-sm">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center border border-cyan-400/20 shadow-inner flex-shrink-0">
                  <span className="text-white text-xs font-black uppercase font-mono">{user?.name ? user.name[0] : 'P'}</span>
                </div>
                <div className="text-left min-w-0 leading-none">
                  <p className="text-xs font-black text-slate-800 dark:text-zinc-200 truncate leading-none">{user?.name || 'Patient'}</p>
                  <span className="text-[9px] text-cyan-600 dark:text-cyan-400 font-bold uppercase tracking-wider block mt-1">PATIENT</span>
                </div>
              </div>
              <button 
