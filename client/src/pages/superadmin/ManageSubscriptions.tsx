...
export const ManageSubscriptions = () => {
  ...
  const totalRevenue = subs.reduce((s, sub) => s + (sub.pricePerUser * (sub.hospital?._count?.memberships || 1)), 0);
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-sm text-slate-500">
                    ...
                    <span>{s.hospital?._count?.memberships || 0} users</span>
                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> Expires {new Date(s.endDate).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
              <span className={`flex items-center gap-1 text-sm font-bold px-3 py-1.5 rounded-full ${s.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
