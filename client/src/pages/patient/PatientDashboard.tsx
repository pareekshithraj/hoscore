  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {showAppointmentAlert && (
        <div className="p-6 rounded-xl bg-amber-50/50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
          <div className="relative z-10 flex items-center gap-4">
            <div className="w-10 h-10 bg-amber-100 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg flex items-center justify-center text-amber-600 dark:text-amber-400 flex-shrink-0">
              <AlertCircle className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-sm font-extrabold text-slate-900 dark:text-white tracking-tight">Recommended Follow-up Visit</h3>
              <p className="text-xs text-slate-600 dark:text-slate-300 font-semibold mt-1">
          ...
          <div className="flex items-center gap-3 relative z-10">
            <button
              ...
              className="px-4 py-2 border border-[var(--card-border)] bg-[var(--inner-bg)] hover:bg-[var(--card-bg)] text-[var(--text-secondary)] text-xs font-bold rounded-lg transition-all active:scale-95 cursor-pointer"
            >
              Skip Alert
            </button>
            <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 border border-[var(--card-border)] bg-gradient-to-br from-cyan-900/20 via-sky-900/10 to-slate-900/40 backdrop-blur-xl rounded-2xl p-6 relative overflow-hidden transition-all duration-300 shadow-xl shadow-cyan-500/5">
          ...
          <div className="grid grid-cols-2 gap-y-4 gap-x-2 border-t border-[var(--card-border)] pt-5">
            <div>
              <span className="text-[8px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-0.5">BLOOD GROUP</span>
              <span className="text-xs font-extrabold flex items-center gap-1 text-[var(--text-primary)]">
                <Heart className="w-3.5 h-3.5 text-rose-500 inline" /> {data.profile?.bloodGroup || 'O+'}
              </span>
            </div>
            <div>
              <span className="text-[8px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-0.5">HEALTH STATUS</span>
            ...
            <div>
              <span className="text-[8px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-0.5">GENDER</span>
              <span className="text-xs font-bold uppercase font-mono text-[var(--text-secondary)]">
                {data.profile?.gender || 'MALE'}
              </span>
            </div>
            <div>
              <span className="text-[8px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-0.5">DATE OF BIRTH</span>
          ...
          <div className="mt-6 pt-4 border-t border-[var(--card-border)] flex items-center justify-between">
            <div className="flex items-center gap-1.5 opacity-70">
              ...
              <span className="text-[10px] font-bold text-[var(--text-muted)]">Verified Patient Identity</span>
            </div>
            <span className="text-[9px] font-mono text-[var(--text-muted)] font-bold">Hoscore Network</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 flex flex-col justify-between self-stretch gap-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl relative overflow-hidden flex-1 shadow-sm">
            <div>
              <h1 className="text-xl lg:text-2xl font-black tracking-tight leading-none text-[var(--text-primary)]">
                Welcome back, {(data.profile?.name || user?.name || 'Patient').split(' ')[0]} 👋
              </h1>
              <p className="text-xs font-semibold mt-2.5 text-[var(--text-secondary)]">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 border border-[var(--card-border)] bg-[var(--inner-bg)] px-3 py-2 rounded-lg">
                <Users className="w-3.5 h-3.5 text-sky-400" />
                <select
                  value={selectedPatientId || ''}
                  onChange={(e) => setSelectedPatientId(e.target.value || null)}
                  className="bg-transparent border-0 text-xs font-bold outline-none cursor-pointer p-0 pr-6 text-[var(--text-primary)]"
                >
                  <option value="" className="bg-white dark:bg-zinc-900 text-slate-900 dark:text-zinc-100">{user?.name} (Primary Profile)</option>
                  {dependents.map((dep) => (
                    <option key={dep.id} value={dep.id} className="bg-white dark:bg-zinc-900 text-slate-900 dark:text-zinc-100">{dep.name}</option>
                  ))}
                </select>
              </div>
              ...
              <button
                ...
                className="px-3.5 py-2 border border-[var(--card-border)] bg-[var(--inner-bg)] hover:bg-[var(--card-bg)] text-[var(--text-secondary)] text-xs font-bold rounded-lg transition-all active:scale-95 cursor-pointer"
              >
                Manage Family
              </button>
              ...
              <Link
                to="/patient/find"
                className="px-4 py-2 bg-[var(--text-primary)] hover:opacity-90 text-[var(--bg-secondary)] text-xs font-bold rounded-lg transition-all active:scale-95 text-center"
              >
                Book Appointment
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="md:col-span-7 border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl p-5 relative overflow-hidden group shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-1">Scheduled Consultations</span>
                  <p className="text-3xl font-black text-[var(--text-primary)] tracking-tight tabular-nums mt-1">{upcomingCount}</p>
                </div>
                <div className="w-8 h-8 bg-sky-500/10 border border-sky-500/20 rounded-lg flex items-center justify-center text-sky-600 dark:text-sky-400">
              ...
              <div className="mt-6 pt-4 border-t border-[var(--card-border)] flex items-center justify-between">
                {data.upcoming && data.upcoming.length > 0 ? (
                  <div className="flex items-center gap-2.5">
                    ...
                    <p className="text-xs text-[var(--text-secondary)] font-bold truncate max-w-[280px]">
                      Next: <span className="text-[var(--text-primary)]">{data.upcoming[0].doctor?.name}</span> ({data.upcoming[0].time})
                    </p>
                  </div>
                ) : (
                  <p className="text-xs text-[var(--text-muted)] font-semibold">No upcoming appointments scheduled</p>
                )}
            ...
            <div className="md:col-span-5 border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl p-5 relative overflow-hidden group shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-1">Active Prescriptions</span>
                  <p className="text-3xl font-black text-[var(--text-primary)] tracking-tight tabular-nums mt-1">{prescriptionsCount}</p>
                </div>
                <div className="w-8 h-8 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400">
                  <FileText className="w-4 h-4" />
                </div>
              </div>
              <div className="mt-6 pt-4 border-t border-[var(--card-border)] flex items-center justify-between">
                {data.recentRx && data.recentRx.length > 0 ? (
                  <p className="text-xs text-[var(--text-secondary)] font-bold truncate max-w-[180px]">
                    Latest Rx: <span className="text-[var(--text-primary)]">{data.recentRx[0].diagnosis}</span>
                  </p>
                ) : (
                  <p className="text-xs text-[var(--text-muted)] font-semibold">No active prescriptions</p>
                )}
        ...
        <div className="border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6 border-b border-[var(--card-border)] pb-4">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-extrabold text-[var(--text-primary)] tracking-tight">Upcoming Consultation Slots</h2>
            </div>
          ...
          {data.upcoming?.length === 0 ? (
            <div className="py-16 text-center">
              <div className="w-12 h-12 bg-[var(--inner-bg)] border border-[var(--card-border)] rounded-lg flex items-center justify-center mx-auto mb-4 text-[var(--text-muted)]">
                <Calendar className="w-6 h-6" />
              </div>
              <p className="text-xs text-[var(--text-secondary)] font-bold tracking-tight">No upcoming consultations found</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Book an appointment online above to schedule a visit.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {data.upcoming?.map((a: any) => (
                <div key={a.id} className="flex items-center gap-4 p-4 rounded-lg border border-[var(--card-border)] bg-[var(--inner-bg)] hover:bg-[var(--card-bg)] hover:border-sky-300 dark:hover:border-sky-500/20 hover:-translate-y-0.5 transition-all duration-200">
                  <div className="w-8 h-8 bg-sky-500/10 border border-sky-500/25 rounded-lg flex items-center justify-center text-sky-600 dark:text-sky-400 flex-shrink-0">
                    <Clock className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-extrabold text-[var(--text-primary)] truncate">{a.doctor?.name || 'Medical Specialist'}</p>
                    <p className="text-[11px] text-[var(--text-secondary)] font-medium truncate mt-0.5">{a.hospital?.name} · {a.time}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-wider ${a.status === 'CONFIRMED'
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 text-[8px] font-black text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/25 rounded tracking-wider hover:bg-rose-100 dark:hover:bg-rose-500/25 hover:border-rose-300 dark:hover:border-rose-500/40 transition-all cursor-pointer"
        ...
        <div className="border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6 border-b border-[var(--card-border)] pb-4">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-extrabold text-[var(--text-primary)] tracking-tight">Recent Prescriptions & Rx</h2>
            </div>
          ...
          {data.recentRx?.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-[var(--text-secondary)] font-bold tracking-tight">No active prescriptions yet</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Issued prescriptions will appear here automatically.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {data.recentRx?.map((rx: any) => (
                <div key={rx.id} className="flex items-center gap-4 p-4 rounded-lg border border-[var(--card-border)] bg-[var(--inner-bg)] hover:bg-[var(--card-bg)] hover:border-emerald-300 dark:hover:border-emerald-500/20 hover:-translate-y-0.5 transition-all duration-200">
                  <div className="w-8 h-8 bg-emerald-500/10 border border-emerald-500/25 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400 flex-shrink-0">
                    <FileText className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-extrabold text-[var(--text-primary)] truncate">{rx.diagnosis}</p>
                    <p className="text-[11px] text-[var(--text-secondary)] font-medium truncate mt-0.5">{rx.doctor?.name} · {rx.hospital?.name}</p>
                  </div>
                  <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-wider ${rx.status === 'ISSUED'
      ...
      <Modal isOpen={showFamilyModal} onClose={() => setShowFamilyModal(false)} title="Manage Family Members">
        <div className="space-y-6">
          <div>
            <span className="text-[10px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-3">Registered Dependents</span>
            {dependents.length === 0 ? (
              <p className="text-xs text-[var(--text-muted)] italic">No dependents registered yet.</p>
            ) : (
              <div className="space-y-2.5 max-h-[160px] overflow-y-auto pr-1">
                {dependents.map((dep) => (
                  <div key={dep.id} className="flex items-center justify-between p-3 rounded-lg bg-[var(--inner-bg)] border border-[var(--inner-border)]">
                    <div>
                      <p className="text-xs font-bold text-[var(--text-primary)]">{dep.name}</p>
                      <p className="text-[10px] text-[var(--text-muted)]">HSC-{dep.sixDigitId} · {dep.bloodGroup}</p>
                    </div>
          ...
          <div className="border-t border-[var(--card-border)] pt-4">
            <span className="text-[10px] text-[var(--text-muted)] font-extrabold uppercase tracking-widest block mb-3">Add Dependents</span>
            <form onSubmit={handleAddDependent} className="space-y-3.5">
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <div className="space-y-1">
                  <span className="text-[8px] text-[var(--text-muted)] font-bold uppercase tracking-wider">Date of Birth</span>
                  <input
                    type="date"
                ...
                <div className="space-y-1">
                  <span className="text-[8px] text-[var(--text-muted)] font-bold uppercase tracking-wider">Gender</span>
                  <select
                ...
                <div className="space-y-1">
                  <span className="text-[8px] text-[var(--text-muted)] font-bold uppercase tracking-wider">Blood Group</span>
                  <select
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-[var(--text-primary)] hover:opacity-90 text-[var(--bg-secondary)] rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer"
              >
