...
import { Clock, Shield, Bell, Save, Globe, Hospital, Upload, Camera, CheckCircle } from 'lucide-react';
...
export const Settings = () => {
  ...
  const [hospitalData, setHospitalData] = useState({ name: '', address: '', city: '', state: '', contact: '', description: '', logo: '' });
  ...
  useEffect(() => {
    ...
    // Load hospital data
    api.get('/hospital/current').then(setHospitalData).catch(() => {});
  }, []);
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" value={hospitalData.city || ''} onChange={e => setHospitalData({...hospitalData, city: e.target.value})}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase mb-1.5 block">State</label>
                  <input type="text" value={hospitalData.state || ''} onChange={e => setHospitalData({...hospitalData, state: e.target.value})}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
                </div>
